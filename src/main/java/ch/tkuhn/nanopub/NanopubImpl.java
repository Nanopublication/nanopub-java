...
public class NanopubImpl implements Nanopub, Serializable {
    ...
    private static final URI SUB_GRAPH_OF = 
            new URIImpl("http://www.w3.org/2004/03/trix/rdfg-1/subGraphOf");
    private static final URI CREATION_TIME = 
            new URIImpl("http://purl.org/dc/terms/created");
    private static final URI DATETIME_TYPE = 
            new URIImpl("http://www.w3.org/2001/XMLSchema#dateTime");
    private static final URI HAS_AUTHOR = 
            new URIImpl("http://swan.mindinformatics.org/ontologies/"
            + "1.2/pav/authoredBy");
    private static final URI HAS_CREATOR = 
            new URIImpl("http://swan.mindinformatics.org/ontologies/"
            + "1.2/pav/createdBy");
    ...
    private static List<RDFFormat> unsuitableFormats = 
            ImmutableList.of(RDFFormat.NTRIPLES, 
    ...
    public NanopubImpl(Collection<Statement> statements) 
    ...
    private static final String nanopubViaSPARQLQuery =
            ...
            "    graph ?H { { this: np:hasAssertion ?G } " +
            "union { this: np:hasProvenance ?G } " +
            "        union { this: np:hasPublicationInfo ?G } } " +
            "  } union { " +
            "    graph ?H { this: a np:Nanopublication . " +
            "?G rdfg:subGraphOf ?I } . " +
            "    graph ?H { { this: np:hasAssertion ?I } " +
            "union { this: np:hasProvenance ?I } " +
    ...
    public NanopubImpl(SPARQLRepository repo, URI nanopubUri) 
        ...
        try {
            ...
            try {
                String q = nanopubViaSPARQLQuery.
                        replaceAll("@", nanopubUri.toString());
    ...
    public NanopubImpl(File file) throws MalformedNanopubException, 
    ...
    public NanopubImpl(URL url) throws MalformedNanopubException, 
            OpenRDFException, IOException {
	...
	if (pubinfoUri == null) {
            throw new MalformedNanopubException
                    ("No publication info URI found");
	}
	collectSubGraphs(statements);
	collectStatements(statements);
    }

    private void collectNanopubUri(Collection<Statement> statements) 
            throws MalformedNanopubException {
		
        for (Statement st : statements) {
            if (st.getContext() == null) {
                throw new MalformedNanopubException
                        ("Null value for context URI found.");
	    }
            if (st.getPredicate().equals(RDF.TYPE) && 
                    st.getObject().equals(Nanopub.NANOPUB_TYPE_URI)) {
                if (nanopubUri != null) {                
                    throw new MalformedNanopubException
                            ("Two nanopub URIs found");				
                }
    ...
    private void collectGraphs(Collection<Statement> statements) 
            throws MalformedNanopubException {
        
	for (Statement st : statements) {
            if (st.getContext().equals(headUri) 
                    && st.getSubject().equals(nanopubUri)) {
                if (st.getPredicate().equals(Nanopub.HAS_ASSERTION_URI)) {
                    if (assertionUri != null) {
                        throw new MalformedNanopubException
                                ("Two assertion URIs found");
                    }		
                    assertionUri = (URI) st.getObject();
		} 
                else if (st.getPredicate().
                        equals(Nanopub.HAS_PROVENANCE_URI)) {					
                    if (provenanceUri != null) {
                        throw new MalformedNanopubException
                                ("Two provenance URIs found");					
                    }					
                    ...
                    if (pubinfoUri != null) {
                        throw new MalformedNanopubException
                                ("Two publication info URIs found");					
                    }					
    ...
    private void collectSubGraphs(Collection<Statement> statements) 
            throws MalformedNanopubException {
	...
	for (Statement st : statements) {
            if (st.getContext().equals(headUri) && 
                    st.getPredicate().equals(SUB_GRAPH_OF)) {
    ...
    private void addGraphUri(URI uri) throws MalformedNanopubException {
        if (graphUris.contains(uri)) {
            throw new MalformedNanopubException
                    ("Each (sub-)graph needs a separate URI");		
        }
	graphUris.add(uri);	
    }

    private void collectStatements(Collection<Statement> statements) 
            throws MalformedNanopubException {
	...
	for (Statement st : pubinfo) {		
            ...
            if (!st.getPredicate().equals(CREATION_TIME)) {
                continue;                
            }		
            ...
            if (!l.getDatatype().equals(DATETIME_TYPE)) {
                continue;                
            }		
            s = l.stringValue();		
            break;
	...
	for (Statement st : pubinfo) {
            ...
            if (!st.getPredicate().equals(HAS_AUTHOR)) {
                continue;
            }
	...
	for (Statement st : pubinfo) {
            ...
            if (!st.getPredicate().equals(HAS_CREATOR)) {
                continue;
            }
            if (!(st.getObject() instanceof URI)) {
                continue;
